...
def sax_writer(ws=None):
    from openpyxl2.writer.worksheet import write_worksheet_data
    if ws is None:
        ws = make_worksheet()
    out = BytesIO()
...
def lxml_writer(ws=None):
    from openpyxl2.writer.lxml_worksheet import write_worksheet_data
    if ws is None:
        ws = make_worksheet()
...
def styled_sheet():
    from openpyxl2 import Workbook
    from openpyxl2.styles import Font, Style, PatternFill, Color, colors
